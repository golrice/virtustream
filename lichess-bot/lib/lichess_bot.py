...
class ClientLogHandler(logging.Handler):
    """Custom log handler to send log messages to a client."""
    ...
    def connect_to_client(self):
        ...
        try:
            # 连接到客户端（假设客户端在localhost:12345监听）
    ...
    def emit(self, record):
        ...
        if self.client_socket:
            try:
                ...
                self.client_socket.sendall(log_entry.encode('utf-8') + b'\n')
            except Exception as e:
...
def play_game(li: lichess.Lichess,
    ...
    """
    Play a game and send all log messages to a client.
    ...
    # 创建客户端日志处理器
    client_handler = ClientLogHandler()
    ...
    try:
        ...
        with engine_wrapper.create_engine(config, game) as engine:
            ...
            logger.debug(f"is ponder : {can_ponder}")
        ...
        # 确保在函数结束时移除处理器
        logger.removeHandler(client_handler)
...
def fake_think_time(config: Configuration, board: chess.Board, game: model.Game) -> datetime.timedelta:
    ...
    sleep = seconds(0.0)
    # 这里被改了，前面几步棋也会思考
    if config.fake_think_time and len(board.move_stack) > 0:
