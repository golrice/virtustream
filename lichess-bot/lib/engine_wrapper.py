        ...
        try:
            ...
            while time.time() - start_time < total_time:
                try:
                    ...
                    datas = datas.split(',')
                    for data in datas:
                        if data and data not in steps:
                            
                            logger.info(f"[{time.time():.3f}] Received: {data}")
                            steps.add(data)
            ...
            logger.info("Error: Server not available.")
        finally:
            client_socket.close()
            logger.info("Connection closed.")
        logger.info(f"best moves:{best_move}")
        ...
        if steps:
            ...
            for move in steps:
                try:
                    ...
                    user_random_step = move
                    break
                except ValueError:
                    logger.exception(f"Ignoring illegal move {move} on board {board.fen()}")
            logger.info(f"user random step:{user_random_step}")
        if user_random_step:
            logger.info("here is changing best")
            user_move = chess.Move.from_uci(user_random_step)
            best_move.move = user_move
        logger.info(f"changed best{best_move}")
        # logger.info("sock close")
        # else:
        #     logger.info(f"elapsed time({elapsed}) > min_time({min_time})")
